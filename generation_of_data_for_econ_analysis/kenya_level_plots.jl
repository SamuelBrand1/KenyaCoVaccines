...
p_IH_sev = normalize([exp(η_hosp * t) * p_IH[t] for t = 1:50], 1)
p_IH_crit = normalize([exp(η_ICU * t) * p_IH[t] for t = 1:50], 1)
...
t1 = findfirst(deaths_data.dates .== Date(2021, 12, 31))
...
include("gather_inf_data_severe.jl");
include("gather_inf_data_crit.jl");
include("gather_inf_data_deaths.jl");
...
## Gather the inferred mean daily incidences critical
...
plt_deaths = plot(xticks = (xtickpos, xticklab),
    size = (800, 400),
    left_margin = 5mm,
    xlims = (xs_model_fitted_to_data[1] - 3, xs_model_fitted_to_data[end] + 70),
    # ylims = (10, 17),
    legend = :topleft,
...
plot!(plt_deaths, [xs_model_fitted_to_data; collect((xs_model_fitted_to_data[end]+1):(xs_model_fitted_to_data[end]+100))], nb_deaths[1].pred[1:(n_model_fit+100)],
    ribbon = (nb_deaths[1].lb[1:(n_model_fit+100)], nb_deaths[1].ub[1:(n_model_fit+100)]),
...
# savefig(plt_deaths, "generation_of_data_for_econ_analysis/main_plots/reported_deaths.pdf")
