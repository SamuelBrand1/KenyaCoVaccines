...
function perc_pos_to_num_tests_scale(model)
    ...
    idxs = (.~isnan.(prop_pos_mv_av)).&(prop_pos_mv_av .> 0).&(1:length(prop_pos_mv_av) .>= length(prop_pos_mv_av) - 21).&(number_tests_mv_av .> 0)
...
function get_predictions(model::KenyaCoVaccines.CoVAreaModel,projection_date::Date,p_ID;seroreversionrate=0.0,σ = 0.16,ω = 1/180,fitlength = 400)
    ...
    MCMCdraws = get(model.MCMC_results.chain,[:β₀,:β_home,:β_school,:β_other,:β_work,:ϵ,:χ,:p_test,:E₁,:E₂,:E₃,:E₄,:E₅,:E₆,:inc_R_αvar,:time_scale_αvar,:mid_point_αvar,:inc_R_δvar,:time_scale_δvar,:mid_point_δvar,:init_scale])
...
function get_predictions_delta(model::KenyaCoVSD.CoVAreaModel,projection_date::Date,p_ID;seroreversionrate=0.0,σ = 0.16,ω = 1/180,ct_min2 = 0.445,fitlength = 400,p_test_scale=5e-4,scale_δ_transmissibility = 1.0)
    @unpack PCR_cases,sero_cases,baseline_sero_array,PCR_array,sero_sensitivity,sero_specificity,N,M_BB,prob,α,γ,relative_testing_rate = model
    ...
    #Calculate prop_pos to test ratio
    test_fit = perc_pos_to_num_tests_scale(model)
    println("incidence fitting - delta variant")
    for k = 1:length(first(MCMCdraws))
        N₁ = MCMCdraws.P_eff[k]*N

        N₂ = N - N₁
        p = [MCMCdraws.R₀[k],model.α,model.γ,MCMCdraws.ϵ[k],σ,N₁,N₂,ω,MCMCdraws.schooleffect[k],MCMCdraws.ct_min1[k],ct_min2,MCMCdraws.Rα[k],MCMCdraws.time_scale_αvar[k],347.0 + MCMCdraws.mid_point_αvar[k]*30.0,MCMCdraws.Rδ[k]*scale_δ_transmissibility,MCMCdraws.time_scale_δvar[k],406.0 + MCMCdraws.mid_point_δvar[k]*30.0]
        u0 = [N₁,MCMCdraws.E₀[k],0.,0.,0.,0.,N₂,MCMCdraws.E₀[k],0.,0.,0.,0.,0.0,0.0 ]

        juneendpoint = (Date(2021,7,1) - Date(2020,2,20)).value
        #Sero-waning
        sero_array = vcat(baseline_sero_array[1:30],[(1-seroreversionrate)^k for k in 1:600])
        #solve model
        sol = solve(prob, Vern9();tspan = (0,(projection_date - Date(2020,2,20)).value),
                                    u0=u0,
                                    p=p,
                                    saveat = 1,
        ...
        #Calculate effective R
        Reff = [(MCMCdraws.R₀[k]*(t < 24 || t ≥ 319) + MCMCdraws.R₀[k]*MCMCdraws.schooleffect[k]*(t≥ 24 && t< 319))*(1 + KenyaCoVSD.gen_logistic(t;L = MCMCdraws.Rα[k],κ = 1/MCMCdraws.time_scale_αvar[k], x₀ = 347.0 + MCMCdraws.mid_point_αvar[k]*30.0))*(1 + KenyaCoVSD.gen_logistic(t;L = MCMCdraws.Rδ[k],κ = 1/MCMCdraws.time_scale_δvar[k], x₀ = 406.0 + MCMCdraws.mid_point_δvar[k]*30.0))
                    for t = 1:length(ι₁)]

        ct1 = [KenyaCoVSD.ct_kenya(t,MCMCdraws.ct_min1[k]) for t = 1:length(ι₁)]
        ct2 = [KenyaCoVSD.ct_kenya(t,ct_min2) for t = 1:length(ι₁)]
        Rt₁[:,k] .= Reff.*(((ct1.*sol[:S₁][2:end]./N₁).*MCMCdraws.ϵ[k].*ct1) .+ ((ct2.*sol[:S₂][2:end]./N₂).*(1 .- MCMCdraws.ϵ[k]).*ct1))
        Rt₂[:,k] .= Reff.*(((ct1.*sol[:S₁][2:end]./N₁).*(1 .- MCMCdraws.ϵ[k]).*ct2) .+ ((ct2.*sol[:S₂][2:end]./N₂).*MCMCdraws.ϵ[k].*ct2))
...
function condense_prediction(prediction,num_tests,deaths,p_ID,relative_testing_rate)
    ...
    function neg_ll(μ)
        ...
        for (t,D) in enumerate(deaths)
            LL += logpdf(Poisson(exp(μ[1])*max(mean_unscaled_deaths1[t],1e-5) + exp(μ[2])*max(mean_unscaled_deaths2[t],1e-5)),D)
        end
        return -LL
    end
